...
import baselines.atari_ddpg.training as training
from baselines.atari_ddpg.models import Actor, Critic
...
def run(env_id, seed, noise_type, layer_norm, batch_norm, evaluation, **kwargs):
    # Configure things.
    ...
    #import ipdb
    #ipdb.set_trace()
    nb_actions = env.action_space.n#shape[-1]
    '''
    ...
    critic = Critic(nb_actions, layer_norm=layer_norm)
    ...
    training.train(env=env, env_id = env_id, eval_env=eval_env, param_noise=param_noise,
        action_noise=action_noise, actor=actor, critic=critic, memory=memory, batch_norm = batch_norm, layer_norm = layer_norm,**kwargs)
    env.close()
    if eval_env is not None:
        eval_env.close()
    if rank == 0:
...
def parse_args():
    ...
    #LunarLander-v2
    parser.add_argument('--env-id', type=str, default='Seaquest-v0')
    ...
    parser.add_argument('--nb-epochs', type=int, default=1000000)  # with default settings, perform 1M steps total
