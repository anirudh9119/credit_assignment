...
class DDPG(object):
    def __init__(self, actor, critic, memory, observation_shape, action_shape, param_noise=None, action_noise=None,
        ...
        critic_l2_reg=0., actor_lr=1e-4, critic_lr=1e-3, clip_norm=None, reward_scale=1., entropy_weight=0.01):
        # Inputs.
    ...
    def setup_actor_optimizer(self):
        ...
        self.entropy_cost = tf.reduce_mean(tf.reduce_sum(tf.log(self.actor_tf + 1e-6) * self.actor_tf, 1))
        print(self.entropy_weight)
        self.actor_loss = -tf.reduce_mean(self.critic_with_actor_tf) + self.entropy_cost * self.entropy_weight
    ...
    def train(self, train_only_actor=False):
        ...
        rewards = np.asarray([data[2] for data in minibatch])#.astype('float32')
        obs1 = np.asarray([data[3] for data in minibatch])#.astype('float32')
        terminal1 = np.asarray([data[4] for data in minibatch])#.astype('float32')
    ...
    def get_stats(self):
        if self.stats_sample is None:
            ...
            minibatch = self.memory.sample(self.batch_size)
            #obs0, action, reward, obs1, terminal1
            obs0 = np.asarray([data[0] for data in minibatch])
            action = np.asarray([data[1] for data in minibatch])
            #rewards = np.asarray([data[2] for data in minibatch])
            #obs1 = np.asarray([data[3] for data in minibatch])
            #terminal1 = np.asarray([data[4] for data in minibatch])
            #rewards = rewards.reshape([self.batch_size, 1])
            #terminal1 = terminal1.reshape([self.batch_size, 1])
            action = action.reshape([self.batch_size, -1])
            #self.stats_sample = self.memory.sample(batch_size=self.batch_size)
        values = self.sess.run(self.stats_ops, feed_dict={
            self.obs0: obs0,#self.stats_sample['obs0'],
            self.actions: action,#self.stats_sample['actions'],
        })
